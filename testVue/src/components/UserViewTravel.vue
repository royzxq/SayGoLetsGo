...
export default {
  ...
  methods: {
    ...
    getBalance: function(expenses) {
      ...
      for (var i in debts) {
        //alert(i+ ':' + debts[i])
        if (debts[i] !== 0) balance.push([i, debts[i]]);
      }
      return balance;
    },
    ...
    submit: function() {
      ...
      this.$store
        .dispatch("groupTravel/updateTravelGroup", travelgroup)
        .then(() => {
  ...
  mounted: function() {
    this.$store
      .dispatch("groupTravel/fetchTravelGroup", { id: this.$route.params.id })
      .then(() => {
        var member;
